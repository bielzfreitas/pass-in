...
public class EventService {
    //instancia do repositoy | Construtor via lombock
    private final EventRepository eventRepository;
    private final AttendeeService attendeeService;

    //retornando detalhes de um evento
    ...
    //registrando attendee
    public AttendeeIdDTO registerAttendeeOnEvent(String eventId, AttendeeRequestDTO attendeeRequestDTO){
        //verificar se já existe
        this.attendeeService.verifyAttendeeSubscription(attendeeRequestDTO.email(), eventId);
    
        //verificar se tem vaga no evento
        Event event = this.getEventById(eventId);
        List<Attendee> attendeeList = this.attendeeService.getAllAttendeesFromEvent(eventId);
    ...
    //método auxilar para consulta (privado)
    ...
    //método auxiliar para o slug
    //normalizando para separar caracteres de letras e acentos
    //deixar string pura - sem acentos
